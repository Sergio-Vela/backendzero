...
export class PasswordHistoryServiceImpl implements PasswordHistoryService {
     async createPasswordHistory(userId: number, pwHash: string, createdAt: Date): Promise<PasswordHistory> {
        ...
        const createPassword: PasswordHistory = await PasswordHistory.create({userId: userId, pwHash: newPwHash, createdAt });
        return createPassword;
    }

    async getPasswordHistory(userId: number): Promise<{ id: number; pwHash: string; createdAt: Date; }[]> {
        ...
        const pwHisto = await PasswordHistory.findAll({
            where: { userId },
            attributes: ['id', 'pwHash', 'createdAt'],
